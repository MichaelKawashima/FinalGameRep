...
public class PlayerTwoBulletWeapon : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		if(Input.GetKeyDown(KeyCode.KeypadEnter) && (mineWeapon == true || missileWeapon == true))
		{	
			basicWeapon = true; 
			mineWeapon = false; 
			missileWeapon = false;
		...
		if(Input.GetKeyDown(KeyCode.RightShift) && (missileWeapon == true || basicWeapon == true))
		{
			basicWeapon = false; 
			mineWeapon = true; 
			missileWeapon = false;
		...
		if(Input.GetKeyDown(KeyCode.RightControl) && (mineWeapon == true || basicWeapon == true))
		{
			basicWeapon = false; 
			mineWeapon = false; 
			missileWeapon = true;
	...
	void ShootMine()
	{
		if(Input.GetKeyDown(KeyCode.RightShift) && canAttackMine && mineAmmo > 0)
		{
	...
	void ShootMissile()
	{
		if(Input.GetKeyDown(KeyCode.RightControl) && canAttackMissile && missileAmmo > 0)
		{
